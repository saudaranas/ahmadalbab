...
function paragraphs(text) {
  ...
  if (!parts.length) return "<p>Kandungan penuh belum ditambah lagi.</p>";
  return parts.map((p) => {
    const trimmed = p.trim();
    const imgMatch = trimmed.match(/^!\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)$/);
    if (imgMatch) {
      const alt = imgMatch[1], url = imgMatch[2];
      return `<figure class="body-img"><img src="${url}" alt="${esc(alt)}" loading="lazy">${alt ? `<figcaption>${esc(alt)}</figcaption>` : ""}</figure>`;
    }
    ...
    if (vidMatch) {
      ...
      if (ytMatch) return `<div class="body-video"><iframe src="https://www.youtube.com/embed/${ytMatch[1]}" title="Video" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe></div>`;
      return `<div class="body-video"><video controls src="${url}"></video></div>`;
    }
    if (trimmed.startsWith(">")) return `<blockquote>${formatInline(trimmed.slice(1).trim())}</blockquote>`;
